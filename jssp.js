...
function on_get_album_art_done(metadb, art_id, image, image_path) {
	if (art_id === 1) {
		// Artist photo response
		var artistKey = ppt.tf_albumartist.EvalWithMetadb(metadb).toLowerCase();
		if (image) {
			var aw = cover.max_w * 2;
			g_artist_cache[artistKey] = image.Resize(aw, aw, 2);
		} else {
			g_artist_cache[artistKey] = false; // no artist art found, mark as done
		}
		if (!timers.coverDone) {
			timers.coverDone = window.SetTimeout(function () {
				brw.repaint();
...
// Artist image cache (for type=100 artist header rows)
var g_artist_cache = {};         // key: album artist string → image or null
var g_artist_load_requested = {}; // key: album artist string → metadb handle or false
	...
	this.setList = function () {
		...
		*/
		...
		for (i = 0; i < end; i++) {
			...
			// ---- ARTIST HEADER ROW (type 100) ----
			var thisArtist = grptags.split(" ^^ ")[0].toLowerCase();
			var prevArtist = (i > 0) ? this.groups[i-1].groupkey.split(" ^^ ")[0].toLowerCase() : null;
			if (thisArtist !== prevArtist && ppt.showgroupheaders) {
				var _artistSlots = ppt.groupHeaderRowsNumber + 1;
				for (var ar = 0; ar < _artistSlots; ar++) {
					this.rows[r] = new Object();
					this.rows[r].type = 100;
					this.rows[r].metadb = this.groups[i].metadb;
					this.rows[r].albumId = i;
					this.rows[r].albumTrackId = ar;
					this.rows[r].playlistTrackId = s;
					this.rows[r].groupkey = grptags;
					this.rows[r].artistKey = thisArtist;
					this.rows[r].artistSlot = ar;
					this.rows[r].artistSlots = _artistSlots;
					r++;
				}
			}
			...
			if (this.rows.length > 0) {
				...
				for (var i = g_start_; i <= g_end_; i++) {
					...
					case 99: // extra bottom row
						...
						break;
                    case 100:
					if (this.rows[i].artistSlot !== 0) break;
					var _slots = this.rows[i].artistSlots;
					var artistRowH = _slots * ah;
					if (ay > 0 - artistRowH && ay < this.h + artistRowH) {
						var artistKey = this.rows[i].artistKey;
						var artistName = this.rows[i].groupkey.split(" ^^ ")[0].toUpperCase();
						var aImgSize = artistRowH - cover.margin * 4;

						// background
						gr.FillSolidRect(ax, ay, aw, artistRowH - 4, g_color_normal_txt & 0x10ffffff);
						gr.FillSolidRect(ax, ay, aw, 2, g_color_highlight & 0xbbffffff);
						gr.FillSolidRect(ax, ay + artistRowH - 5, aw, 1, g_color_normal_txt & 0x15ffffff);

						// load image from biography cache
						if (typeof g_artist_cache[artistKey] === 'undefined') {
							g_artist_cache[artistKey] = null;
							var _artistName = this.rows[i].groupkey.split(" ^^ ")[0];
							var _firstLetter = _artistName.substring(0, 1).toLowerCase();
							var _bioPath = fb.ProfilePath + "foo_spider_monkey_panel\\package_data\\{BA9557CE-7B4B-4E0E-9373-99F511E81252}\\biography-cache\\art_img\\" + _firstLetter + "\\" + _artistName + "\\";
							var _img = null;
							try {
								var _fso = new ActiveXObject("Scripting.FileSystemObject");
								if (_fso.FolderExists(_bioPath)) {
									var _folder = _fso.GetFolder(_bioPath);
									var _files = new Enumerator(_folder.Files);
									for (; !_files.atEnd(); _files.moveNext()) {
										var _f = _files.item();
										var _ext = _f.Name.split(".").pop().toLowerCase();
										if (_ext == "jpg" || _ext == "png" || _ext == "jpeg") {
											_img = gdi.Image(_f.Path);
											break;
										}
									}
								}
							} catch(e) {}
							g_artist_cache[artistKey] = _img ? _img.Resize(aImgSize, aImgSize, 2) : false;
						}

						var aImg = g_artist_cache[artistKey];
						if (aImg) {
							gr.DrawImage(aImg, ax + cover.margin * 2, ay + cover.margin * 2, aImgSize, aImgSize, 0, 0, aImg.Width, aImg.Height, 0, 230);
							gr.DrawRect(ax + cover.margin * 2, ay + cover.margin * 2, aImgSize - 1, aImgSize - 1, 1.0, g_color_normal_txt & 0x25ffffff);
						} else {
							gr.SetSmoothingMode(2);
							gr.DrawEllipse(ax + cover.margin * 2, ay + cover.margin * 2, aImgSize, aImgSize, 1.5, g_color_normal_txt & 0x30ffffff);
							gr.SetSmoothingMode(0);
						}

						var textX = ax + aImgSize + cover.margin * 5;
						var textW = aw - aImgSize - cover.margin * 7;
						gr.SetTextRenderingHint(4);
						gr.GdiDrawText(artistName, g_font_group1, g_color_normal_txt, textX, ay, textW, artistRowH - 4, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
					}
					break;
					};
		...
		case "down":
			...
			if (!cTouch.down && !timers.mouseDown && this.ishover && this.activeRow > -1 && Math.abs(scroll - scroll_) < 2) {
				...
				case (rowType == 0): // ----------------> track row
					...
					break;
                case (rowType == 100): // ----------------> artist header row
                    this._artistImageContextMenu(x, y, this.rows[this.activeRow]);
                    return true;
...
this._artistImageContextMenu = function (x, y, row) {
		var _menu = window.CreatePopupMenu();
		_menu.AppendMenuItem(MF_STRING, 1, "Set Artist Image...");
		_menu.AppendMenuItem(MF_STRING, 2, "Clear Artist Image");
		var ret = _menu.TrackPopupMenu(x, y);

		var _artistName = row.groupkey.split(" ^^ ")[0];
		var _firstLetter = _artistName.substring(0, 1).toLowerCase();
		var _bioFolder = fb.ProfilePath + "foo_spider_monkey_panel\\package_data\\{BA9557CE-7B4B-4E0E-9373-99F511E81252}\\biography-cache\\art_img\\" + _firstLetter + "\\" + _artistName + "\\";
		var _artistKey = row.artistKey;

			if (ret == 1) {
			var _file = utils.InputBox(window.ID, "Enter the full path to the image file:", "Set Artist Image", "");
			if (_file && _file.length > 0) {
				try {
					...
					fb.ShowPopupMessage("Could not copy image:\n" + e.message, "Artist Image");
				}
			}
		} else if (ret == 2) {
			try {
				var _fso = new ActiveXObject("Scripting.FileSystemObject");
				var _customPath = _bioFolder + "custom.jpg";
				if (_fso.FileExists(_customPath)) {
					_fso.DeleteFile(_customPath);
				}
				delete g_artist_cache[_artistKey];
				brw.repaint();
			} catch(e) {}
